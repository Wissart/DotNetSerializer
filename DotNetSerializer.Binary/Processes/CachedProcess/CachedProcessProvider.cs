...
namespace DotNetSerializer.Binary.Processes.CachedProcess
{
    internal class CachedProcessProvider : ProcessProvider
    {
        ...
        public CachedProcessProvider(BinaryOptions options) 
            : base(options)
        {
            ...
            _cachedSerializerProvider = new CachedSerializerProvider(options);
        }

        public override IDeserializationProcess GetDeserializationProcess()
        {
            var cachedProcessType = _options.CachedProcessSettings.ProcessType;
        ...
        private IDeserializationProcess CreateDeserializationProcess(CachedProcessType cachedProcessType)
        {
            switch (cachedProcessType)
            {
                case CachedProcessType.Single:
                    return new CachedDeserializationProcess(_options, _cachedSerializerProvider);
                case CachedProcessType.Parallel:
                    if (_parallelSerializerProvider == null)
                        _parallelSerializerProvider = new ParallelSerializerProvider(_options, _cachedSerializerProvider);

                    return new ParallelDeserializationProcess(_options, _parallelSerializerProvider);
                default:
        ...
        private ISerializationProcess CreateSerializationProcess(CachedProcessType cachedProcessType)
        {
            ...
            {
                case CachedProcessType.Single:
                    return new CachedSerializationProcess(_options, _cachedSerializerProvider);
                case CachedProcessType.Parallel:
                    if (_parallelSerializerProvider == null)
                        _parallelSerializerProvider = new ParallelSerializerProvider(_options, _cachedSerializerProvider);

                    return new ParallelSerializationProcess(_options, _parallelSerializerProvider);
                default:
                    throw new DotNetSerializerException("Unknown cached process type");
            }
        }
